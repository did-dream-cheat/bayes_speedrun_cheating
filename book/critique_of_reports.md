...
Sir Ronald Fisher himself often presents the p-value as the odds of the null hypothesis being false, instead of the odds of calculating a test statistic more extreme on infinite future data than the null hypothesis predicts. Textbook authors carried the error forward.
...
That simulation within the PE report has a blind spot. First, consider Blaze rods. In order to earn them in Minecraft, you must kill Blazes. While it is possible to kill multiple Blazes at once, the only way that's practical during a speedrun is to use the sweeping edge attack of a sword against one Blaze with multiple other near-death Blazes nearby. On top of the difficulty of achieving that feat, most Minecraft speedrunners never craft a sword. Axes do more damage per strike than a sword, are practically a requirement to gather vital wood, and some speed running stategies never ask the player to attack groups. Nearly always, then, Minecraft runners kill one Blaze at a time, and thus can easily stop when they meet their quota.
